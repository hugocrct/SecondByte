...
const sampleProducts = [
  {
    ...
    price: 499,
    originalPrice: 1099,
    description: "iPhone 12 Pro reacondicionat, bateria al 92%, inclou carregador i cable original. Pantalla i carcassa com a nou.",
    image: null,
  ...
  },
  {
    ...
    price: 699,
    originalPrice: 1299,
    description: "MacBook Air M1, 8GB RAM, 256GB SSD. Lleugeres marques d'ús a la carcassa, teclat i pantalla perfectes.",
    image: null,
    seller: "SecondByte Certified",
    createdAt: new Date().toISOString()
  },
  {
    ...
    price: 299,
    originalPrice: 499,
    description: "NVIDIA RTX 3060 Ti, perfecte estat, només 6 mesos d'ús. Inclou caixa original.",
    image: null,
    seller: "SecondByte Certified",
    createdAt: new Date().toISOString()
  },
  {
    ...
    price: 349,
    originalPrice: 649,
    description: "iPad Air 4, lleugera rascada a la part posterior, pantalla impecable. Bateria al 88%.",
    image: null,
    seller: "SecondByte Certified",
    createdAt: new Date().toISOString()
  }
...
function displayProducts(productsArray) {
  ...
  if (filtered.length === 0) {
    grid.innerHTML = `
      <div class="col-12 text-center py-5">
        <i class="fas fa-box-open fa-3x mb-3" style="color: var(--sb-text-soft);"></i>
        <p class="text-muted">No s'han trobat productes amb aquests filtres</p>
      </div>
    `;
    return;
  }
  ...
  grid.innerHTML = filtered.map(doc => {
    ...
    const imageHtml = hasImage
      ...
      : `<div class="product-image-placeholder"><i class="fas fa-microchip fa-3x" style="color: var(--sb-pink);"></i></div>`;
...
function addToCart(productId) {
  showAlert("Producte afegit al carretó! Properament podràs finalitzar la compra.", "alert-success");
}

// Pujar imatge a Firebase Storage
...
async function publishProduct(productData) {
  try {
    ...
    showAlert("Producte enviat per revisió! Rebràs una resposta en 24h", "alert-success");
    ...
    console.error("Error publicant producte:", error);
    showAlert("Error al publicar el producte", "alert-danger");
  }
}
...
function setupImageUpload() {
  ...
  if (uploadBtn && fileInput) {
    ...
    fileInput.addEventListener("change", async (e) => {
      ...
      if (file && file.type.startsWith("image/")) {
        ...
        showAlert("Si us plau, selecciona una imatge vàlida (JPG, PNG)", "alert-danger");
      }
    });
  }
  ...
  const sampleProducts = [
    ...
    {
      name: "Corsair RM750x",
      category: "font_alimentacio",
      condition: "excellent",
      price: 89,
      originalPrice: 139,
      description: "Font d'alimentació Corsair RM750x 750W 80 Plus Gold. Modular, com a nova.",
  ...
  function displayProducts(productsArray) {
    ...
    if (filtered.length === 0) {
      grid.innerHTML = `
      <div class="col-12 text-center py-5">
        <i class="fas fa-microchip fa-3x mb-3" style="color: var(--sb-text-soft);"></i>
